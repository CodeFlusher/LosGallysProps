...
public class StyleEditor extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (context.getPlayer() == null) {
            InformativeLogger.error("Style editor", "Player is null.");
            InformativeLogger.error("Style editor", new InvalidParameterException());
            return ActionResult.FAIL;
        }
        if (!context.getPlayer().isCreative()) {
            InformativeLogger.debugMessage("Style Editor", "Player must be in creative!");
            return ActionResult.SUCCESS;
        }
        if (block instanceof StyledBlock) {
            SwitchListener listener = new SwitchListener(input -> {
                PacketByteBuf buf = PacketByteBufs.create();
                buf.writeBlockPos(pos);
                buf.writeInt((int) input);
                ClientPlayNetworking.send(LosGallysPropsNetworking.STYLE_CHANGE_PACKET_ID, buf);
            });
    ...
    public void setCustomScreen(WorldAccess world, BlockState state, BlockPos pos, SwitchListener listener) {
        InformativeLogger.debugMessage("Style Editor", "Screen opened.");
