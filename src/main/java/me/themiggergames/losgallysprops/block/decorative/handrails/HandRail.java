...
public class HandRail extends HorizontalFacingBlock implements Waterloggable {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        if (inverse) {
            return this.getDefaultState().with(Properties.HORIZONTAL_FACING, ctx.getHorizontalPlayerFacing().getOpposite()).with(WATERLOGGED, false);
        } else {
            return this.getDefaultState().with(Properties.HORIZONTAL_FACING, ctx.getHorizontalPlayerFacing()).with(WATERLOGGED, false);
        }
    }
